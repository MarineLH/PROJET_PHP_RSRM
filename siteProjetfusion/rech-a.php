                ...
                <?php 
                    ...
                    print('<h4>Sélectionnez le (ou les) domaine(s) de votre recherche.</h4>');
                     $result = mysqli_query($dbconn, "SELECT * FROM domaine");
                        ...
                        print("</tr></table>");
                        print('<input type="submit" name="recherche" value="Rechercher"/>');
                    ...
                    else
                    {
                        if(isset($_POST['recherche'])){  
                            ...
                            {
                                $SQLQuery = mysqli_query($dbconn,"SELECT org_nom, org_ville, org_codepostal, rec_datelimiteintervention, rec_description, dom_libelle, rec_titre, rec_id
                               ...
                               WHERE rec_active = 1 AND dom_id = ".$value);
                                ...
                                {
                                    print('<tr>');
                                    print('<td>'.$row['dom_libelle'].'</td><td>'.$row['org_nom'].'</td><td>' .$row['org_ville'].'</td><td>'.$row['org_codepostal'].'</td><td>'.$row['rec_titre'].'</td><td>'.$row['rec_datelimiteintervention'].'</td><td>'.$row['rec_description'].'</td><td><a href="contact.php?annID='.$row['rec_id'].'">Répondre</a></td></tr>');
                                }
                            }
                            print('</table></form>');
                        }
